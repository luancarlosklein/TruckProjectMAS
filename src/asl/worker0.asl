...
/* Initial beliefs*/
id(0).
capacity(2). //Maximum weight the agent can carry
...
<-  .time(H, M, S);
	...
	?box(WeightBox, Local);
	-+dropLocal(Local);
	//This is the posibility the agent make a mistake in the truck, spend more time here
	.random(R);
	?jumbled(X);
	if(R < X)
	{
