...
<-  .wait(1000);
	.print("The truck is empty now! Nothing tho discharge here (T3)! Let's generate another");
	.concat("+restartTruck(true)",Event);
    .at("now +7 seconds", Event).

//Make this fot call a worker
+!doANewCnp(X): qtdThings(Y) & Y > 0
<-  .print("I have a box! Is someone to discharge?(T3)");
	.wait(3000);
	!startCNP(X).
...
+!doANewCnp(X): true
<- .wait(5000);
	.print("The doNewCnp not work! Do it again! (T3)");
   !startCNP(X).

+!generateNextBox([]): true
<- 	.print("The truck is empty now! Nothing tho discharge here (T3)!").  
...
<-  
	...
	//.my_name(Me);
	//.send([truck1, truck2], tell, impression(Me, Ag, Ti, [Capa, Time], [0,0]));
	.print("I need a new CNP! The winner agent is busy now! (T3)");
	.wait(3000);
	!startCNP(6).
   ...
   -+qtdOffers(S);
    if(S > 0)
    {
    	...
    	+proposalWin(CNPId, P, Time, WAg);
   		.print("Winner is ",WAg," with ",WOf, " Time: ", Time, " T3");
   		!announce_result(CNPId,L,WAg); 
   		-+cnp_state(CNPId,finished);
    }   
   -winnerCnp(_,_);
   .abolish(proposeA(6,_,_,_));
   .abolish(propose(6,_,_));
   .abolish(refuse(6)).
...
<- .print("CNP has failed!: No prosose (T3)");
   .wait(7000);
   !startCNP(6).

-!contract(CNPId): qtdThings(Y) & Y > 0
<- .print("CNP ",CNPId," has failed! (T3)");
   .wait(7000);
   !startCNP(6).
 
-!contract(CNPId): qtdThings(Y) & Y <= 0
<- .print("CNP ",CNPId," has failed! Stop now!(T3)").
...
<-  
	.send(WAg,tell,accept_proposal(CNPId, truck3));     
...
<- 	
	...
	.my_name(Me);
	.send([truck1, truck2], tell, impression(Me, A, Ti, [Capa, Time], [J,CapaScore]));
	-leaving(TIME, WeightBox)[source(A)];
	-proposalWin(CNPId, Capa, Time, A).
//////////////////////////////////////////////////////////////////////////////////
//Actualize the image and the reputation
