...
public class CalculateReputation extends DefaultInternalAction {
   ...
   public Object execute(TransitionSystem ts, 
	 ...
	 // double totali = addAll(totalTime, allImpressions);
	  ...
	  {	  
		  ...
		  //System.out.println("EOOOOOOOOOOOOOOOOOO222---> " + impressionsW2);
		  
	     
	  //Delete the old reputations
	  ts.getAg().delBel(Literal.parseLiteral("reputation(worker0,_,_)"));
	  ts.getAg().delBel(Literal.parseLiteral("reputation(worker1,_,_)"));
	  ts.getAg().delBel(Literal.parseLiteral("reputation(worker2,_,_)"));
	  int i = 0;
	  while (i < 3)
	  {
		  ts.getAg().addBel(Literal.parseLiteral("reputation(" + workes[i] + "," + reputations1[i] + "," + reputations2[i] + ")"));
		  i += 1;
	  }
